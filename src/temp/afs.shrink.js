...
Od = (a, b = 1, c = 1) => (b ?
  `<div class="thumb" data-col="${a[t]}" data-fill="${a[oa]}"><div class="crd ab" style="${a[V] ? `background-image:url(img/${a[V]}.webp)` : w}"></div>
  <div class="fll fl"></div><div class="gl"></div><div class="de"></div><div class="ld"></div><div class="n1"></div><div class="n2"></div><div class="n3"></div><div class="n4"></div></div>` : w)
...
Qi = {
  ...
  [Qb]: (b, a) => {
    b[B] =
      `<div id="drop-top"><h3>Congratulations! You've found:</h3><h2>${l[a.k].n}</h2></div>
      <div id="drop-img" class="${l[a.k].t} ${a.k}" ${a.r ? `style="transform:rotate(${a.r}deg)"` : w}>
        ${ve(a)}
      </div>
      <div id="drop-caption">
        <h4>level ${l[a.k][e]} ${l[a.k].t || Na}${l[a.k].t != ib && a[t] ? ` in ${l[a[t]].n}` : w}</h4>
        <p>${l[a.k][j]}</p>
        <button onClick="${fb.name}()">${x(Wh)}</button>
      </div>`;
    pb(Qb, .3);
  },
  ...
  ach: a => {
    if (b.achQ[0]) {
      ...
      a[B] =
        `<div id="ach-top"><h3>Congratulations! You've achieved:</h3><h2>${e.n}</h2></div>
        <div id="ach-img" class="ach-${c} ach-l${d}">
          <img src="img/${d ? 'medal' + d : 'trophy'}.webp">
          <span class="ach-ico">${e.ico}</span>
        </div>
        <div id="ach-caption">
          <h4>${b.ach[c].v || ' '} ${e.sub || w}</h4>
          <p>${e[j]}<br> </p>
          <button onClick="${fb.name}()">${x(sj)}</button>
        </div>`;
      pb('ach', .5);
      if (c != cd) {
        // Bonus.
  ...
  [Xb]: (q, r, o = w, f = w, a, n) => {
    ...
    b[P][i]((a, f, i, e = hc((mb() - a.ts) / 3600), h = hc((mb() - a.ts) / 60) - e * 60,
      ...
      if (!a[Yb]) {// Don't show sculptures.
        o += `<div class="F-stats" style="outline: 2px dashed ${l[a[t]][t]}">
        <div class="F-id">#${f + 1}</div>
        <div class="F-n"><span style="border-bottom: 2px dashed ${l[a[Za]][t]}">${a.n}</span></div>
        ${b('Colony') + Ae(a)}</div>
        ...
        ${b() + Uc(c(p), 'connection')}. (<span class="num">${a[v][k](a => a[u] > 0 && a[u] < 100)[g]}</span> WIP)</div>
        </div>`;
      }
    });
    // Awards output.
    ...
    `<div id="scores-scroll">
      <div id="scores-wrapper">
        ...
        <div id="scoreHead">Total score: ${b[U]} (inc. bonuses)<span class="date">${new Date().toLocaleDateString('en-US', {weekday: 'long', year: Qj, month: 'long', day: Qj})}</span></div>
        <div id="scores" class="stats">${o}</div>
      </div>
    </div>
    <div id="awards-wrapper">
      <h2><span>・✦ = Feathers in your hat = ✦・</span></h2>
      <div id="awards" class="stats">${f || '<p class="no-ach">It looks like you have achieved very little.</p>'}</div>
    </div>
    ...
    <div id="volume">🔊<input type="range" min="1" max="100" value="${b.vol}" id="vol"></div>
...
ze = (c, a, b, d, e, f = w) => `<h2>${d}</h2><div id="${a}-form" class="select-form"><form ${f}>
    ${!ma(b)[g] ? '<div class="no-opts">No options available</div>' : Mg(b).map(([b, c]) => `<label><input type="radio" name="${a}" value="${b}">${c}</label>`)[Ab](w)}
    </form><button ${ma(b)[g] ? w : fe} onClick="${Ri.name}(${c}, '${a}')">${e}</button>
  </div>`,
...
Ef = (e, b = e[Hd], a = Ce(b)) => {
  ...
  if (b[d][zb](Kc)) b[mi](Ff);
    ...
    if (a[Ue]) {
      ...
      ['wing-l', 'wing-r'][i](e => {
        Wa(c(zd), `<div id="${a.id + e}" style="transform:rotate(${a.r}deg);left:${d}px;top:${f}px;width:${b.clientWidth}px;height:${b.clientHeight}px;" class="leaf alate ${Vc(a)}">
          <div style="transform:${getComputedStyle(ga(`#${a.id} .body`))[de]}" class="body"><div class="wings"><div class="wing ${e}"></div></div></div></div>`);
